...
public class SnarkApplication
{
    public static void main (String[] args)
    {
        ...
        final Snark snark = parseArguments(args, new ConsoleStorageReporter(), null);
        ...
        // add a shutdown hook for each sybil as well
        /*SnarkShutdown sybilHook1 = new SnarkShutdown(sybilSnark1, listener);
        Runtime.getRuntime().addShutdownHook(sybilHook1);
        SnarkShutdown sybilHook2 = new SnarkShutdown(sybilSnark1, listener);
        Runtime.getRuntime().addShutdownHook(sybilHook2);*/
        ...
        // wait till the storage is created/allocated
        while (snark.storage == null) {}
              
        // create sybils and do this for each sybil
        Thread t2 = new Thread("Sybil-1") {
        	public void run () {
        		try {
        			log.log(Level.INFO, "KPN-MN: inside sybil-1 thread");
        			Snark sybilSnark1 = snark.copyContentsTo();
        			sybilSnark1.collectPieces();
        		} catch (IOException ioe) {
        			System.exit(-1);
        		}
        ...
        Thread t3 = new Thread("Sybil-2") {
        	public void run () {
        		try {
        			log.log(Level.INFO, "KPN-MN: inside sybil-2 thread");
        			Snark sybilSnark2 = snark.copyContentsTo();
        			sybilSnark2.collectPieces();
        		} catch (IOException ioe) {
        			System.exit(-1);
        		}
    ...
    protected static void doInteractive (Snark snark, SnarkShutdown hook)
    {
        ...
        try {
            ...
            log.log(Level.SEVERE, "KPN-MN: Unable to read stdin", ioe);
        }
