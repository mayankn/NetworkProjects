...
public class ConsoleStorageReporter implements StorageListener
{
    ...
    {
        log.log(Level.FINE, "KPN-MN: Creating file '" + name + "' of length " +
            length + ": ");
    }
    ...
    public void storageAllocated (Storage storage, long length)
    {
        ...
        if (allocated == storage.getMetaInfo().getTotalLength()) {
            System.err.println();
            log.log(Level.INFO, "KPN-MN: Finished allocating storage space");
        }
    }
    ...
    public void storageChecked (Storage storage, int num, boolean checked)
    {
        if (!allChecked && !checking) {
            ...
            if (meta != null) {
                log.log(Level.INFO, "KPN-MN: Checking existing " + meta.getPieces()
                    + " pieces: ");
            }
            checking = true;
        }
        if (checking) {
            ...
            log.log(Level.FINE, "KPN-MN: Got " + (checked ? "" : "BAD ") + "piece: "
                + num);
        }
    }
