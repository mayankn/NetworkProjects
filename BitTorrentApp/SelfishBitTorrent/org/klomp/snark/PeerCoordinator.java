...
public class PeerCoordinator implements PeerListener
{
    ...
    public void connected (Peer peer)
    {
        ...
        synchronized (peers) {
            if (peerIDInList(peer.getPeerID(), peers)) {
                log.log(Level.FINER, "KPN-MN: Already connected to: " + peer);
                peer.disconnect(false); // Don't deregister this
                // connection/peer.
            } else {
                log.log(Level.FINER, "KPN-MN: New connection to peer: " + peer);
    ...
    public void addPeer (final Peer peer)
    {
        if (peer != null) {
            ...
            if (need_more) {
                ...
                if (peer.isConnected()) {
                    log.log(Level.FINER, "KPN-MN: Add peer already connected: " + peer);
                } else {
                    log.log(Level.FINER, "KPN-MN: MAX_CONNECTIONS = " + MAX_CONNECTIONS
    ...
    void unchokePeer ()
    {
        ...
        while (it.hasNext()) {
            Peer peer = (Peer)it.next();
            // at the start add peers who are also interesting to us
        ...
        while (uploaders < MAX_UPLOADERS && interested.size() > 0) {
            ...
            log.log(Level.FINER, "KPN-MN: Unchoke: " + peer);
        ...
        throws IOException
    {
        ...
        synchronized (wantedPieces) {
            ...
            if (!wantedPieces.contains(p)) {
                log.log(Level.FINER, "KPN-MN: " + peer + " piece " + piece
                    + " no longer needed");
            ...
            try {
                if (storage.putPiece(piece, bs)) {
                    log.log(Level.FINER, "KPN-MN: Recv p" + piece + " " + peer);

                    // keep track of how many pieces we received from this peer and then upload not more than that to the same peer
                    peer.state.propSharePieces++;
                    
                } else {
                    // Oops. We didn't actually download this then... :(
                    downloaded -= metainfo.getPieceLength(piece);
                    log.log(Level.INFO, "KPN-MN: Got BAD piece " + piece + " from "
                        + peer);
                    
                    // and we must also choke this peer for sending a bad piece
        ...
        synchronized (peers) {
            ...
            while (it.hasNext()) {
                Peer p = (Peer)it.next();
                // announce only to interesting connected peers about the piece
                if (p.isConnected() && p.isInteresting()) {
                    p.have(piece);
                }
            }
        }
        ...
        if (completed()) {
        	double timeTaken = (double) (System.nanoTime() - SnarkApplication.startTime)/1000000000;
        	log.log(Level.INFO, "KPN-MN: Completed download in : " + Double.toString(timeTaken) + " seconds");
            client.interrupt();
            System.exit(0);
        }
        return true;
    ...
    {
        log.log(Level.FINER, "KPN-MN: Got choke(" + choke + "): " + peer);
    ...
    {
        log.log(Level.FINER, "KPN-MN: Disconnected " + peer);
