...
public class SnarkShutdown extends Thread
{
    ...
    public void run ()
    {
        log.log(Level.INFO, "KPN-MN: Shutting down...");

        log.log(Level.FINE, "KPN-MN: Halting ConnectionAcceptor...");
        if (snark.acceptor != null) {
            snark.acceptor.halt();
        }

        log.log(Level.FINE, "KPN-MN: Halting TrackerClient...");
        if (snark.trackerclient != null) {
            snark.trackerclient.halt();
        }

        log.log(Level.FINE, "KPN-MN: Halting PeerCoordinator...");
        if (snark.coordinator != null) {
            snark.coordinator.halt();
        }

        log.log(Level.FINE, "KPN-MN: Closing Storage...");
        if (snark.storage != null) {
            try {
                snark.storage.close();
            } catch (IOException ioe) {
                log.log(Level.SEVERE, "KPN-MN: Couldn't properly close storage", ioe);
            }
        }
