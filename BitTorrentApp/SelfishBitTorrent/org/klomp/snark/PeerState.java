...
class PeerState
{
    ...
    // receiving part size
    private final static int PARTSIZE = 65535; // 64K
    
    // sending part size
    private final static int SEND_PARTSIZE = 16384; // 16K
    
    // PropShare counter to ensure upload proportional to download
    int propSharePieces = 0;
    ...
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " rcv alive");
        /* XXX - ignored */
    }

    void chokeMessage (boolean choke)
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " rcv " + (choke ? "" : "un") + "choked");
    ...
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " rcv " + (interest ? "" : "un")
            + "interested");
        interested = interest;
    ...
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " rcv have(" + piece + ")");
        // Sanity check
        if (piece < 0 || piece >= metainfo.getPieces()) {
            // XXX disconnect?
            log.log(Level.FINER, "KPN-MN: Got strange 'have: " + piece
                + "' message from " + peer);
            return;
        }
    ...
    {
        synchronized (this) {
            log.log(Level.FINEST, "KPN-MN: " + peer + " rcv bitfield");
            if (bitfield != null) {
                // XXX - Be liberal in what you accept?
                log.log(Level.FINER, "KPN-MN: Got unexpected bitfield message from "
                    + peer);
                return;
            }
    ...
    void requestMessage (int piece, int begin, int length)
        throws IOException
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " rcv request(" + piece + ", " + begin
            + ", " + length + ") ");
        if (choking) {
            log.log(Level.FINER, "KPN-MN: Request received, but choking " + peer);
            return;
        }
        ...
        if (piece < 0 || piece >= metainfo.getPieces() || begin < 0
            ...
            || length > 2 * SEND_PARTSIZE) {
            // XXX - Protocol error -> disconnect?
            log.log(Level.FINER, "KPN-MN: Got strange 'request: " + piece + ", "
                + begin + ", " + length + "' message from " + peer);
            
            // choke peer if requests a piece greater than SEND_PARTSIZE
        ...
        if (pieceBytes == null) {
            ...
            log.log(Level.FINER, "KPN-MN: Got request for unknown piece: " + piece);
            return;
        }
        ...
        if (begin >= pieceBytes.length || begin + length > pieceBytes.length) {
            // XXX - Protocol error-> disconnect?
            log.log(Level.FINER, "KPN-MN: Got out of range 'request: " + piece + ", "
                + begin + ", " + length + "' message from " + peer);
            return;
        }

        log.log(Level.FINEST, "KPN-MN: Sending (" + piece + ", " + begin + ", "
            + length + ")" + " to " + peer);
        
        // do not send more pieces than what we received from this peer
        if (this.propSharePieces > 0) {
        	out.sendPiece(piece, begin, length, pieceBytes);
        	this.propSharePieces--;
        }

        // Tell about last subpiece delivery.
        if (begin + length == pieceBytes.length) {
            log.log(Level.FINEST, "KPN-MN: Send p" + piece + " " + peer);
        }
    }
        ...
        throws IOException
    {
        ...
        if (getFirstOutstandingRequest(req.piece) == -1) {
            if (listener.gotPiece(peer, req.piece, req.bs)) {
                log.log(Level.FINEST, "KPN-MN: Got " + req.piece + ": " + peer);
            } else {
                log.log(Level.FINEST, "KPN-MN: Got BAD " + req.piece + " from " + peer);
                // XXX ARGH What now !?!
                // already punished the peer by choking him, nothing to do here
                downloaded = 0;
            }
        }
    }
    ...
    {
        log.log(Level.FINEST, "KPN-MN: getChunk(" + piece + "," + begin + "," + length
            + ") " + peer);
        ...
        if (r == -1) {
            log.log(Level.FINER, "KPN-MN: Unrequested 'piece: " + piece + ", " + begin
        ...
        synchronized (this) {
            ...
            if (req.piece != piece || req.off != begin || req.len != length) {
                log.log(Level.FINER, "KPN-MN: Unrequested or unneeded 'piece: " + piece
            ...
            if (r != 0) {
                ...
                errmsg += " " + peer;
                log.log(Level.FINER, "KPN-MN: " + errmsg);
            }
    ...
    {
        log.log(Level.FINEST, "KPN-MN: Got cancel message (" + piece + ", " + begin
            + ", " + length + ")");
    ...
    {
        log.log(Level.WARNING, "KPN-MN: Ignoring unknown message type: " + type
            + " length: " + bs.length);
    }

    void havePiece (int piece)
    {
        log.log(Level.FINEST, "KPN-MN: Tell " + peer + " havePiece(" + piece + ")");
    ...
    private boolean requestNextPiece ()
    {
        ...
        if (bitfield != null) {
            ...
            log.log(Level.FINEST, "KPN-MN: " + peer + " want piece " + nextPiece);
            synchronized (this) {
                if (nextPiece != -1
    ...
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " setInteresting(" + interest + ")");
    ...
    {
        log.log(Level.FINEST, "KPN-MN: " + peer + " setChoking(" + choke + ")");
